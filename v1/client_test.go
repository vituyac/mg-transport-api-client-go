...
func (t *MGClientTest) Test_ProductMessagesHistory() {
	c := t.client()
	...
	snd := SendHistoryMessageRequest{
		Message: SendMessageRequestMessage{
			...
			Product: &MessageDataProduct{
				...
				Name:    "Product name",
				Article: "Product article",
				Url:     "https://example.local/product/1",
				Img:     "https://example.local/product/1/img",
				Cost: &MessageDataOrderCost{
					Value:    100,
					Currency: "USD",
				},
	...
	t.gock().
		...
		Filter(func(request *http.Request) bool {
			...
			var actual SendHistoryMessageRequest
			t.Require().NoError(json.Unmarshal(data, &actual))

			return t.Assert().Equal(snd.Message.Product, actual.Message.Product)
		}).
		Reply(http.StatusOK).
		JSON(
			MessagesResponse{
